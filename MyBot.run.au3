...
Func InitializeMBR(ByRef $sAI, $bConfigRead)
	...
	Local $cmdLineHelp = GetTranslatedFileIni("MBR GUI Design - Loading", "Commandline_multiple_Bots", "By using the commandline (or a shortcut) you can start multiple Bots:\r\n" & _
			...
			"Supported Emulators are MEmu, Nox, BlueStacks2, BlueStacks.\r\n\r\n" & _
			"Examples:\r\n" & _
...
Func MainLoop($bCheckPrerequisitesOK = True)
	...
	Local $diffhStarttime
	
	While 1

		If $g_bMustRunBotLoop = True And $g_bRunState Then
			$g_bMustRunBotLoop = False
			$g_iBotAction = $eBotStart
			_runBot()
			ContinueLoop
		EndIf
		...
		$diffhStarttime = _Timer_Diff($hStarttime)
...
Func runBot()
	$g_bMustRunBotLoop = True
EndFunc   ;==>runBot

Func _runBot() ;Bot that runs everything in order
	...
	If ProfileSwitchAccountEnabled() And $g_bReMatchAcc Then
		...
		If $g_bMustRunBotLoop = True Then Return
	EndIf
